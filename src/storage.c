...
struct TransferTicket *g_StorageConfValues;

//Redo all this... directory creation should not be in filename generation
static char *
generateFilename (struct Block *block, char *basepath)
{
    ...
    if ((filename =
         malloc (hashlength + strlen (strLength) + strlen (dir) + 3)) == NULL)
    {
...
generateURL (struct Block *block)
{
    ...
    if (asprintf (&dir, "%c/%c/%c/%c", hash[0], hash[1], hash[2], hash[3]) ==
        -1)
    {
        rzb_log (LOG_ERR, "%s: Could not allocate directory string",
                 __func__);
...
static uint32_t
readWrap (int fd, uint8_t * data, uint64_t length)
{
...
SO_PUBLIC uint32_t
ticketSize (struct TransferTicket * ticket)
{

    return (sizeof (ticket->filesize) + sizeof (ticket->localityId) +
            sizeof (ticket->retrievalType) + sizeof (ticket->port) +
            strlen (ticket->basePath) + strlen (ticket->hostName) + 2);
}


#if 0
This information is required by the disptacher to process a 
block removal.  When we flesh out this feature the locality data
will be stored in the database.

static uint32_t
StoreDataTicket (const char *filename, void *data, uint64_t size)
{
    ...
    {
        rzb_log (LOG_ERR,
                 "StoreDataAsFile: Could not allocate ticket filename string");
        free (ticketfilename);
        return 0;
    }

    fd = open (ticketfilename, O_RDWR | O_CREAT | O_TRUNC,
...
RetrieveDataTicket (const char *filename)
{
    ...
    char *ticketfilename;
    void *data;
    struct TrasferTicket *ticket;

    if (asprintf (&ticketfilename, "%s.ticket", filename) == -1)
    {
        rzb_log (LOG_ERR, "%s: Could not allocate ticket filename string",
                 __func__);
        free (ticketfilename);
        return 0;
    }
    ...
    {
        rzb_perror
            ("RetrieveDataTicket: Could not open file for writing: %s");
        free (ticketfilename);
        return 0;
    }
...
binaryTicketSize (char *data)
{
    ...
    hostNamelen += 1;
    rzb_log (LOG_DEBUG, "%s:  Hostname is %s %d", __func__, data + nonstrlen,
             strlen (data + nonstrlen));
    uint32_t basePathlen = strlen (data + hostNamelen + nonstrlen);
    basePathlen += 1;
    rzb_log (LOG_DEBUG, "%s:  Base path is %s", __func__,
             data + nonstrlen + hostNamelen);
...
#endif 
static uint32_t
StoreDataAsFile (struct Block *block, struct TransferTicket *ticket)
{
    ASSERT (block != NULL);
...
static uint32_t
RetrieveDataAsFile (struct Block *block,
...
SO_PUBLIC struct TransferTicket *
StoreDataBlock (struct Block * block)
{
    struct TransferTicket *ticket;
    if ((ticket = calloc(1,sizeof(struct TransferTicket))) == NULL)
        return NULL;
    ...
    if (asprintf(&ticket->basePath, "%s", g_StorageConfValues->basePath) == -1)
    {
        Storage_TicketDestroy(ticket);
        return NULL;
    }
    if (asprintf(&ticket->hostName, "%s",  g_StorageConfValues->hostName) == -1)
    {
        Storage_TicketDestroy(ticket);
        return NULL;
    }
    ...
    if (StoreDataAsFile (block, ticket) == 0)
    {
        Storage_TicketDestroy(ticket);
        return NULL;
    }
...
SO_PUBLIC void
Storage_TicketDestroy(struct TransferTicket *ticket)
{
    ASSERT(ticket != NULL);
    if (ticket == NULL)
        return;

    if (ticket->hostName != NULL)
...
SO_PUBLIC uint32_t
RetrieveDataBlock (struct Block * block, struct TransferTicket *ticket)
{
    ...
    if ( (ticket->localityId == Config_getLocality () ) && 
            (ticket->localityId != 0) )
    {
        return RetrieveDataAsFile (block, ticket);
    }
    else
    {
        ...
        {
        case 0:
            return RetrieveOverCurl (block, ticket);
...
This code is to be revisited.

Ticket information will be populated from the database.
...
SO_PUBLIC uint32_t
RemoveDataBlock (struct Block * block)
{
...
readStorageConf ()
{
    ...
    config_setting_lookup_int (list, "Locality",
                               (conf_int_t *) & g_StorageConfValues->
                               localityId);
    config_setting_lookup_int (list, "Type",
                               (conf_int_t *) & g_StorageConfValues->
                               retrievalType);
    config_setting_lookup_int (list, "Port", (conf_int_t *) & port);
    g_StorageConfValues->port = port;
    config_setting_lookup_string (list, "Basepath", &basepath);
    config_setting_lookup_string (list, "Hostname", &hostname);

    if ((g_StorageConfValues->basePath =
         (char *) malloc (strlen (basepath) + 1)) == NULL)
    {
        rzb_log (LOG_ERR, "%s: Malloctile Dysfunction", __func__);
        return false;
    }

    if ((g_StorageConfValues->hostName =
         (char *) malloc (strlen (hostname) + 1)) == NULL)
    {
        rzb_log (LOG_ERR, "%s: Malloctile Dysfunction", __func__);
        return false;
    }
...
Data_Storage_Initialize ()
{
    ...
    if ((g_StorageConfValues =
         calloc (1,sizeof(struct TransferTicket)))== NULL)
    {
        rzb_log (LOG_ERR, "%s: Malloctile Dysfunction", __func__);
        return false;
    }
