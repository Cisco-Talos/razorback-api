...
SO_PUBLIC uint32_t ticketSize (struct MessageInspectionTicket *ticket) {
...
static uint32_t StoreDataAsFile (struct Block *block) {
    ...
    if ((block->pTicket = serializeTicket(&ticket)) == NULL) {
        rzb_log(LOG_ERR, "%s: Could not serialize ticket", __func__);
        free(filename);
        return 0;
    }

    block->ticketSize = ticketSize(&ticket);
    rzb_log(LOG_DEBUG, "Storage TSize: %d", block->ticketSize);

    if (StoreDataTicket(filename, block->pTicket, block->ticketSize) == 0) {
...
SO_PUBLIC uint32_t binaryTicketSize(char *data) {
    uint32_t nonstrlen = sizeof(uint64_t) + sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint16_t);
    uint32_t hostNamelen = strlen(data + nonstrlen);
    hostNamelen += 1;
    rzb_log(LOG_DEBUG, "%s:  Hostname is %s %d", __func__, data+nonstrlen, strlen(data + nonstrlen));
    uint32_t basePathlen = strlen(data + hostNamelen + nonstrlen);
    basePathlen += 1;
    rzb_log(LOG_DEBUG, "%s:  Base path is %s", __func__, data+nonstrlen+hostNamelen);
...
static uint32_t RetrieveOverCurl (struct Block *block, struct MessageInspectionTicket *ticket) 
{
    ...
    if (curl_easy_setopt(CURLinstance, CURLOPT_WRITEDATA, (void *)data) != 0) {
...
SO_PUBLIC uint32_t RetrieveDataBlock (struct Block *block) {
    ...
    struct MessageInspectionTicket *ticket;
    if ((ticket = deserializeTicket(block->pTicket, block->ticketSize)) == NULL) {
        rzb_log(LOG_ERR, "%s: Could not deserialize ticket", __func__);
        return 0;
    }
//TODO: localityId needs to be unhardcoded
    if (ticket->localityId == Config_getLocality() && ticket->localityId != 0) 
