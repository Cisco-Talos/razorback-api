...
Inspection_Launch (struct RazorbackContext *p_pContext, uint32_t initThreads, uint32_t maxThreads)
{
    ...
    //nugName =
        //UUID_Get_NameByUUID (p_pContext->uuidApplicationType, UUID_TYPE_NUGGET);
    threadName = NULL;
    if (asprintf (&threadName, "Inspection Thread") == -1)
    {
    ...
    //free(nugName);
    p_pContext->pInspectionThreadPool = ThreadPool_Create(
    ...
    if (p_pContext->pInspectionThreadPool == NULL)
    {
...
Inspection_Thread (struct Thread *p_pThread)
{
    ...
    if (l_pContext->pInspectionHooks->initThread != NULL)
    {
    	if (!l_pContext->pInspectionHooks->initThread(&threadData))
    	{
    ...
    if (l_pContext->pInspectionHooks->cleanupThread != NULL)
        l_pContext->pInspectionHooks->cleanupThread(threadData);
