...
BlockPool_Init(struct RazorbackContext *p_pContext)
{
    ...
    {
        rzb_log(LOG_ERR, "%s: Failed to load magic file", __func__);
...
BlockPool_FinalizeItem (struct BlockPoolItem *p_pItem)
{
    ...
    if (uuid_is_null(p_pItem->pEvent->pBlock->pId->uuidDataType) == 1 && p_pItem->pDataHead != NULL)
    {
        ...
        pthread_mutex_unlock(&sg_mMagicMutex);
        if (!UUID_Get_UUID(l_sMagicFull, UUID_TYPE_DATA_TYPE,p_pItem->pEvent->pBlock->pId->uuidDataType))
            return false;
    }
...
SO_PUBLIC bool
BlockPool_Metadata_Add_Filename(struct BlockPoolItem *block, const char * fileName)
{
    return Metadata_Add_Filename(block->pEvent->pMetaDataList, fileName);
}
