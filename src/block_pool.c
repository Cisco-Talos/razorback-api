...
BlockPool_CreateItem(struct RazorbackContext *context)
{
    ...
    if ((item = calloc(1, sizeof(struct BlockPoolItem))) == NULL) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: Failed to allocate new item", __func__);
        return NULL;
    }

    if ((item->pEvent = Event_Create()) == NULL) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: Failed to allocate new event", __func__);
        free(item);
        return NULL;
    }
...
BlockPool_AddData_FromFile(struct BlockPoolItem *item, char *fileName, bool tempFile){
    ...
    if ((dataBlock = calloc(1, sizeof(struct BlockPoolData))) == NULL) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: failed to allocate data time", __func__);
        return false;
    }
    ...
    if ((dataBlock->data.file = fopen(fileName, "r")) == NULL) {
        rzb_perror(LOG_C_CORE, "WTF: %s");
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: failed to open file: %s", __func__, fileName);
        BlockPool_DestroyItemDataList(dataBlock);
        return false;
    }
    if (fstat(fileno(dataBlock->data.file), &sb) == -1) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: failed to stat file: %s", __func__, fileName);
        BlockPool_DestroyItemDataList(dataBlock);
        return false;
    }
    ...
    if ((Config_getBlockPoolMaxSize() > 0) &&
            ((sg_size + sb.st_size) >= Config_getBlockPoolMaxSize())) {
    ...
    if (item->iStatus != BLOCK_POOL_STATUS_COLLECTING) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: failed: item not collecting", __func__);
    ...
    if ((Config_getBlockPoolMaxItemSize() > 0) &&
        ...
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: Block pool item size limit exceeded", __func__);
        BlockPool_Item_Unlock(item);
        Mutex_Lock(sg_sizeMutex);
        sg_size -= sb.st_size;
        Mutex_Unlock(sg_sizeMutex);
                               ...
                               uint32_t length, int flags)
{
    ...
    if ((Config_getBlockPoolMaxItemSize() > 0) &&
        ((item->pEvent->pBlock->pId->iLength + length) >= Config_getBlockPoolMaxItemSize())) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: Block Pool Item Size exceeded", __func__);
        BlockPool_Item_Unlock(item);
        Mutex_Lock(sg_sizeMutex);
        sg_size -= length;
        Mutex_Unlock(sg_sizeMutex);
...
BlockPool_FinalizeItem (struct BlockPoolItem *p_pItem) {
    ...
    if (!Hash_Finalize(p_pItem->pEvent->pBlock->pId->pHash)) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: Failed to finalize hash", __func__);
        return false;
    }
    ...
    while (l_pData != NULL) {
        ...
        switch (l_pData->iFlags) {
            ...
            case BLOCK_POOL_DATA_FLAG_MANAGED:
                break;
            default:
                rzb_log(LOG_ERR, LOG_C_CORE, "%s: Failed to free block data", __func__);
                break;
        }
        Mutex_Lock(sg_sizeMutex);
...
BlockPool_ForEachItem(int (*function) (struct BlockPoolItem *, void *), void *userData) {
    ...
    if (function == NULL) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: function is NULL", __func__);
        return;
    }
...
static void BlockPool_Delete(void *a) {
    ASSERT(a != NULL);
    if (a == NULL) {
        rzb_log(LOG_ERR, LOG_C_CORE, "%s: a is NULL", __func__);
        return;
    }
