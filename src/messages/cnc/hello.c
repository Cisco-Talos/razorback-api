...
MessageHello_Initialize (struct RazorbackContext *context)
{
    ...
    if ((context->iFlags & CONTEXT_FLAG_DISPATCHER) == CONTEXT_FLAG_DISPATCHER)
    {
        message->flags = context->dispatcherFlags;
        message->priority = context->dispatcherPriority;
        message->port = context->dispatcherPort;
        message->protocol = context->dispatcherProtocol;
        if( (message->addressList = List_Clone(context->dispatcherAddressList)) == NULL)
        {
            Hello_Destroy(msg);
            return NULL;
        }
