...
SO_PUBLIC bool 
Metadata_Add_Port (struct NTLVList *list, uuid_t name, uint8_t *port)
{
    uuid_t uuidType;
    ...
    return Metadata_Add(list, name, uuidType, 2, port);
    return true;
}
...
SO_PUBLIC bool 
Metadata_Add_MalwareName (struct NTLVList *list, const char *name)
{
    uuid_t uuidName;
...
SO_PUBLIC bool 
Metadata_Add_Port_Source (struct NTLVList *list, uint8_t *addr)
{
    uuid_t uuidName;
    if (!UUID_Get_UUID(NTLV_NAME_SOURCE, UUID_TYPE_NTLV_NAME, uuidName))
    {
...
SO_PUBLIC bool 
Metadata_Add_Port_Destination (struct NTLVList *list, uint8_t *addr)
{
    uuid_t uuidName;
    if (!UUID_Get_UUID(NTLV_NAME_DEST, UUID_TYPE_NTLV_NAME, uuidName))
    {
