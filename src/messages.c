                                      ...
                                      struct Judgment *p_pJudgment)
{
    ...
    p_pMessage->pJudgment = p_pJudgment;
    ...
    p_pMessage->mhHeader.iLength =
        MessageHeader_BinaryLength (&p_pMessage->mhHeader) +
        (uint32_t) sizeof (p_pMessage->iReason) +
        Judgment_BinaryLength(p_pMessage->pJudgment);
    p_pMessage->mhHeader.iVersion = MESSAGE_VERSION_1;
                                   ...
                                   *p_pMessage)
{
    ...
    if (p_pMessage->pJudgment != NULL)
        Judgment_Destroy(p_pMessage->pJudgment);
                         ...
                         struct EventId *p_pEventId)
{
    ...
    // fill in rest of message
    // calculate header
    p_pMessage->mhHeader.iType = MESSAGE_TYPE_LOG;
    p_pMessage->mhHeader.iLength =
        MessageHeader_BinaryLength (&p_pMessage->mhHeader) +
    ...
    if (p_pEventId != NULL)
        p_pMessage->mhHeader.iLength += sizeof(struct EventId);
