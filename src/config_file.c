...
testFile (const char *configfile)
{
    ...
    if (fd == -1)
    {
        rzb_log (LOG_EMERG, "%s: Failed to open (%s) in ", __func__, configfile);
        return false;
    }
...
getConfigArray (config_t *config, const char *configFile, const char *name)
{
	...
	{
		rzb_log (LOG_ERR, "%s: failed to lookup name: %s", __func__, config_error_text (config));
		return NULL;
	}
...
getNextConfigElem (config_t *config, config_setting_t *list)
{
	...
	while (currItem < numItems)
    {
                                      ...
                                      currItem)) != NULL)
		{
            ...
            rzb_log (LOG_ERR, "%s: Failed to get element: %s", __func__, config_error_text (config));
			break;
		}
	}
