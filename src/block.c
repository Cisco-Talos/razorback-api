...
Block_Destroy (struct Block *p_pBlock)
{
    ...
    if (p_pBlock->pTicket != NULL)
		free (p_pBlock->pTicket);
...
Block_Clone (const struct Block *p_pSource)
{
    ...
    l_pDestination->ticketSize = p_pSource->ticketSize;

	if (p_pSource->ticketSize == 0)
		l_pDestination->pTicket = NULL;
	else 
	{
        if ((l_pDestination->pTicket =
             malloc (p_pSource->ticketSize)) == NULL)
        {
            rzb_log (LOG_ERR, "%s: failed due to lack of memory", __func__);
            Block_Destroy (l_pDestination);
            return NULL;
        }
        memcpy (l_pDestination->pTicket, p_pSource->pTicket,
                p_pSource->ticketSize);
	}
