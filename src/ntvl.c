              ...
              uuid_t p_uuidType, uint32_t p_iSize, const uint8_t * p_pData)
{
    ...
    if (l_pTempItem == NULL)
    {
        rzb_log (LOG_ERR, "NTLVList_Add failed due to out of memory on item");
        return false;
    };
    ...
    if (l_pTempListItem == NULL)
    {
        free (l_pTempItem);
        rzb_log (LOG_ERR,
                 "NTLVList_Add failed due to out of memory on list item");
        return false;
    }
    ...
    if (l_pTempItem->pData == NULL)
    {
        ...
        rzb_log (LOG_ERR,
                 "NTLVList_Add failed due to out of memory on item data");
        return false;
    };
...
NTLVList_Create (void)
{
    ...
    {
        rzb_log(LOG_ERR, "Failed to allocate memory for new ntlv list.");
        return NULL;
    }
    l_pList->iCount=0;
    // set head to NULL
...
NTLVList_Copy (struct NTLVList *p_pDest, const struct NTLVList *p_pSource)
{
    ...
    while (l_pListItem != NULL)
    {
        ...
        {
            rzb_log (LOG_ERR,
                     "NTLVList_Copy failed due to failure of NTLVList_Add");
            return false;
        };
