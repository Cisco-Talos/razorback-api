              ...
              uuid_t type, uint32_t size, const uint8_t * data)
{
    ...
    {
        oldData = item->pData;
        if ((item->pData = calloc (size, sizeof (uint8_t))) == NULL)
        {
            item->pData = oldData;
            rzb_log (LOG_ERR,
                     "%s: failed due to out of memory on item data, old item remains", __func__);
            return false;
        }
        free(oldData);
    }
...
NTLV_Cmp(void *a, void *b)
{
    ...
    if (a == b)
        return 0;
    if ( (uuid_compare(iA->uuidName, iA->uuidName) == 0) ||
