                                   ...
                                   uint32_t initialThreads, uint32_t maxThreads)
{
    ...
    context->iFlags = 0;
    context->iDataTypeCount = dataTypeCount;
    context->pDataTypeList = dataTypeList;
    context->pCommandHooks = NULL;
    context->pInspectionHooks = inspectionHooks;
    ...
    if ((context->judgmentQueue = Queue_Create(JUDGMENT_QUEUE, QUEUE_FLAG_SEND, Razorback_Get_Message_Mode())) == NULL)
    {
        Razorback_Remove_Context(context);
        return false;
    }
                                   ...
                                   uuid_t applicationType)
{
    ...
    context->iFlags = 0;
    context->pCommandHooks = NULL;
    context->pInspectionHooks = NULL;
    context->outputThreads = List_Create(LIST_MODE_GENERIC,
            Thread_Cmp, 
            Thread_KeyCmp, 
            NULL, // destroy
            NULL, //Clone
                                   ...
                                   uuid_t applicationType)
{
    ...
    context->iDataTypeCount = 0;
    context->pDataTypeList = NULL;
    context->pCommandHooks = NULL;
    context->pInspectionHooks = NULL;
...
Razorback_Shutdown_Context (struct RazorbackContext *context)
{
    ...
    if (context->pInspectionThreadPool != NULL)
        ThreadPool_KillWorkers(context->pInspectionThreadPool);
...
Razorback_Render_Verdict (struct Judgment *judgment)
{
    ...
    Queue_Put (context->judgmentQueue, message);
...
Razorback_Output_Launch (struct RazorbackContext *context, struct RazorbackOutputHooks *hooks)
{
    ...
    List_Push(context->outputThreads, thread);
    return true;
}
