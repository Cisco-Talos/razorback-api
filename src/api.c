        ...
        if (context == l_pNode->pContext)
        {
            ...
            else if (l_pNode == sg_ContextList.pTail)
            {
                ...
                if (l_pPrev != NULL)
                    ...
                    rzb_log(LOG_ERR, "%s: Removing item from end of list in first iteration and item was not also list head.", __func__);
            }
            else if (l_pPrev != NULL)
                l_pPrev->pNext = l_pNode->pNext;
            else
                rzb_log(LOG_ERR, "%s: Failed to remove item from list", __func__);
...
Razorback_Init_Context (struct RazorbackContext *p_pContext)
{
    ...
    {
        if ((p_pContext->judgmentQueue = JudgmentQueue_Initialize(QUEUE_FLAG_SEND)) == NULL)
        {
            Razorback_Remove_Context(p_pContext);
            return false;
        }
...
Razorback_Shutdown_Context (struct RazorbackContext *context)
{
    ...
    if (context->judgmentQueue != NULL)
        JudgmentQueue_Terminate(context->judgmentQueue);
...
Razorback_Inspection_Thread (struct Thread *p_pThread)
{
    ...
    {
        if ((l_misMessage = InspectorQueue_Get (l_pQueue)) == NULL)
        {
            // timeout
        ...
        {
            JudgmentQueue_Put (l_pContext->judgmentQueue, l_mjsMessage);
