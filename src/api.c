...
Razorback_Inspection_Thread (struct Thread *p_pThread)
{
    ...
    {
        rzb_log (LOG_ERR, "%s: Failed to create ntlv list",
                 __func__);
        return;
    }
    ...
    while (true)                // TODO: This should not be for ever and ever ever.
    {
		...
		if (l_misMessage.mhHeader.iType == MESSAGE_TYPE_TICKET) {
        ...
        MessageJudgmentSubmission_Initialize (&l_mjsMessage, l_pBlock,
                                              l_pEventId,
