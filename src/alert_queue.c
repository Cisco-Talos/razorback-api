...
AlertQueue_Get (void)
{
    ...
    {
        rzb_log(LOG_ERR, "%s: Failed to allocate message", __func__);
        return NULL;
    }
    ...
    if (!BinaryBuffer_Get_uint32_t (l_pBuffer, &message->iDisposition))
    {
        BinaryBuffer_Destroy (l_pBuffer);
        rzb_log (LOG_ERR,
    ...
    if (!BinaryBuffer_Get_UUID (l_pBuffer, message->uuidInspectorId))
    {
        BinaryBuffer_Destroy (l_pBuffer);
        rzb_log (LOG_ERR,
    ...
    if (!BinaryBuffer_Get_Block (l_pBuffer, &message->pBlock))
    {
        BinaryBuffer_Destroy (l_pBuffer);
        rzb_log (LOG_ERR,
