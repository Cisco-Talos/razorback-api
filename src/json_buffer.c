...
JsonBuffer_Put_NTLVItem (struct NTLVItem *p_pItem, json_object *parent )
{
    ...
    if (str == NULL)
    {
        ...
        if (!JsonBuffer_Put_String(object, "String_Value", (char *)p_pItem->pData))
            return LIST_EACH_ERROR;
                                     ...
                                     struct List **p_pList)
{
    ...
    for (int i = 0; i < json_object_array_length(parent); i++)
    {
