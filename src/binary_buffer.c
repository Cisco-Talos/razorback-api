...
bool
BinaryBuffer_Put_TransferTicket (struct BinaryBuffer *buffer, struct TransferTicket *ticket)
{


    if (BinaryBuffer_Put_uint64_t (buffer, ticket->filesize) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize file size", __func__);
        return false;
    }

    if (BinaryBuffer_Put_uint32_t (buffer, ticket->localityId) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize localityId", __func__);
        return false;
    }

    if (BinaryBuffer_Put_uint32_t (buffer, ticket->retrievalType) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize retrievalType", __func__);
        return false;
    }

    if (BinaryBuffer_Put_uint16_t (buffer, ticket->port) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize port", __func__);
        return false;
    }

    if (BinaryBuffer_Put_String (buffer, (uint8_t *) ticket->hostName) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize hostName", __func__);
        return false;
    }

    if (BinaryBuffer_Put_String (buffer, (uint8_t *) ticket->basePath) == false)
    {
...
bool
BinaryBuffer_Get_TransferTicket (struct BinaryBuffer *buffer, struct TransferTicket **ret)
{

    struct TransferTicket *ticket;

    if ((ticket = malloc (sizeof (struct TransferTicket))) == NULL)
    {
        rzb_log (LOG_ERR, "%s: Malloctile Dysfunction", __func__);
        return false;
    }

    if (BinaryBuffer_Get_uint64_t (buffer, &ticket->filesize) == false)
    {
    ...
    if (BinaryBuffer_Get_uint32_t (buffer, &ticket->localityId) == false)
    {
    ...
    if (BinaryBuffer_Get_uint32_t (buffer, &ticket->retrievalType) == false)
    {
        rzb_log (LOG_ERR, "%s: Could not deserialize retrievalType",
                 __func__);
        free (ticket);
        return false;
    }

    if (BinaryBuffer_Get_uint16_t (buffer, &ticket->port) == false)
    {
    ...
    if ((ticket->hostName = (char *) BinaryBuffer_Get_String (buffer)) == NULL)
    {
    ...
    if ((ticket->basePath = (char *) BinaryBuffer_Get_String (buffer)) == NULL)
    {
        rzb_log (LOG_ERR, "%s: Could not serialize basePath", __func__);
