...
BinaryBuffer_Put_uint64_t (struct BinaryBuffer * p_pBuffer, uint64_t p_iValue)
{
    ...
    if (p_pBuffer->iOffset + (uint64_t) sizeof (uint64_t) >
        p_pBuffer->iLength)
    {
        rzb_log (LOG_ERR, "BinaryBuffer_Put_uint64_t failed due to overrun");
        return false;
    }
    // place in the buffer in network order
                           ...
                           uint64_t * p_pValue)
{
    ...
    if (p_pBuffer->iOffset + (uint64_t) sizeof (uint64_t) >
        p_pBuffer->iLength)
    {
        rzb_log (LOG_ERR, "BinaryBuffer_Get_uint64_t failed due to overrun");
        return false;
    }
    // read the value
    *p_pValue =
                          ...
                          struct BlockId * p_pId)
{
    ...
    {
        rzb_log (LOG_ERR,
                 "BinaryBuffer_Put_BlockId failed due failure of BinaryBuffer_Put_uint64_t");
        return false;
    }
                          ...
                          struct BlockId * p_pId)
{
    ...
    if (!BinaryBuffer_Get_uint64_t (p_pBuffer, &p_pId->iLength))
    {
        rzb_log (LOG_ERR,
                 "BinaryBuffer_Get_BlockId failed due failure of BinaryBuffer_Get_uint64_t");
        return false;
    }
    if ((p_pId->pHash =
    ...
    else
    {
        ...
        else 
        {
            ...
            while (l_pData != NULL)
            {
                ...
                {
                    rzb_log(LOG_ERR, "buf: %d, off: %d, data: %d", p_pBuffer->iLength, p_pBuffer->iOffset, p_pBlock->bidId.iLength);
                    rzb_log (LOG_ERR,
                             "BinaryBuffer_Put_Block failed due failure of BinaryBuffer_Put_ByteArray");
                    return false;
                }
