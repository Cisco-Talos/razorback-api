                        ...
                        struct Block * p_pBlock)
{
	...
	if (!BinaryBuffer_Put_uint16_t (p_pBuffer, p_pBlock->ticketSize)) {
		rzb_log (LOG_ERR,
				 "%s: failed due failure of BinaryBuffer_Put_uint16_t", __func__);
		return false;
	}

	if (p_pBlock->ticketSize != 0) {
        if (!BinaryBuffer_Put_ByteArray
              (p_pBuffer, p_pBlock->ticketSize, p_pBlock->pTicket))
        {
             rzb_log (LOG_ERR,
                     "%s: failed due failure of BinaryBuffer_Put_ByteArray", __func__);
             return false;
                        ...
                        struct Block ** p_pBlock)
{
	...
	if (l_pBlock->isStored != 1) {
        if (!BinaryBuffer_Get_uint16_t (p_pBuffer, &l_pBlock->ticketSize))
        {
            rzb_log (LOG_ERR,
                     "%s: failed due failure of BinaryBuffer_Get_uint16_t", __func__);
            Block_Destroy(l_pBlock);
            *p_pBlock = NULL;
            return false;
        }
        if (l_pBlock->ticketSize != 0) {

            if ((l_pBlock->pTicket = malloc (l_pBlock->ticketSize)) == NULL)
            {
                rzb_log (LOG_ERR,
                         "BinaryBuffer_Get_Block failed due failure of malloc");
                Block_Destroy(l_pBlock);
                *p_pBlock = NULL;
                return false;
            ...
            if (!BinaryBuffer_Get_ByteArray
                (p_pBuffer, l_pBlock->ticketSize, l_pBlock->pTicket))
            {
                rzb_log (LOG_ERR,
                         "BinaryBuffer_Get_Block failed due failure of BinaryBuffer_Get_ByteArray");
                Block_Destroy(l_pBlock);
