    ...
    for (;;)
    {
        ...
        switch (scpacket.msgtype)
        {
            ...
            case REQBLOB:
                ...
                {
                    perror("Failed to handle blob request\n");
                    return R_FAIL;
                }
                break;
            case RST:
...
static HRESULT handleSendBlob(RZBNetworkConnection *net, ALERT *alert)
{
    ...
    if (recvWrap(net, &req, sizeof(req)) == R_SUCCESS)
    {
        if (sendWrap(net, &scpacket, sizeof(scpacket)) != R_SUCCESS)
        {
            printf("Failed to send snakecharm\n");
            return R_FAIL;
        }
    }
    else
    {
        printf("Failed to receive blob request packet\n");
        return R_FAIL;
    }
    ...
    switch (req.blob_type)
    {
        ...
        case NORM_BLOCK:
            ...
            break;
        default:
            printf("In handleSendBlob, incorrect data type: %u\n", req.blob_type);
            return R_FAIL;
    }
    ...
    if (sendWrap(net, &resp, sizeof(resp)) == R_SUCCESS)
    {
        if (sendWrap(net, data, resp.blobSize) == R_SUCCESS)
        {
            if (recvWrap(net, &scpacket, sizeof(scpacket)) == R_SUCCESS)
            {
                ...
                {
                    printf("Non-Ack message received: %u\n", scpacket.msgtype);
                    return R_FAIL;
                }
                return R_SUCCESS;
            }
