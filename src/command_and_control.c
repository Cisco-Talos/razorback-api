...
static union CCMessage sg_ccmuMessage;
...
CommandAndControl_Start (struct RazorbackContext *p_pContext)
{
        ...
        CONTEXT_FLAG_STAND_ALONE)
    {
        if (!sg_bQueueInitialized)
        {
            ...
            if ((sg_readQueue = CommandQueue_Initialize (QUEUE_FLAG_RECV)) == NULL)
            {
                rzb_log (LOG_ERR, "%s: C&C Error: Failed to connect to MQ.", __func__);
                return false;
            }
            if ((sg_writeQueue = CommandQueue_Initialize (QUEUE_FLAG_SEND)) == NULL)
            {
                rzb_log (LOG_ERR, "%s: C&C Error: Failed to connect to MQ.", __func__);
                return false;
            }
...
CommandAndControl_Thread (struct Thread *p_pThread)
{
    ...
    {
        if (!CommandQueue_Get (sg_readQueue, &sg_ccmuMessage))
        {
            // timeout
        ...
        {
            if (Razorback_LookupContext (sg_ccmuMessage.
    ...
    CommandQueue_Terminate(sg_readQueue);
    CommandQueue_Terminate(sg_writeQueue);
...
#endif //CNC_DEBUG
        return CommandAndControl_processHelloMessage (sg_ccmuMessage.mhHello);
...
CommandAndControl_SendBye (struct RazorbackContext *context)
{
    ...
    uuid_t bCast;
    uuid_clear(bCast);
    if (( bye.mbBye = MessageBye_Initialize(
            context->uuidNuggetId, bCast)) == NULL)
    {
        rzb_log(LOG_ERR, "%s: Failed to create bye message", __func__);
        return false;
    }

    if (!CommandQueue_Put (sg_writeQueue, &bye))
    {
        MessageBye_Destroy(bye.mbBye);
...
{
    union CCMessage l_mrrRegReq;

    if ((l_mrrRegReq.mrrRegReq = MessageRegistrationRequest_Initialize (
    ...
    if (!CommandQueue_Put (sg_writeQueue, &l_mrrRegReq))
    {
        rzb_log (LOG_ERR,
                 "%s: C&C Register: Failed to send registration Request", __func__);
        MessageRegistrationRequest_Destroy (l_mrrRegReq.mrrRegReq);
        return false;
    }
    MessageRegistrationRequest_Destroy (l_mrrRegReq.mrrRegReq);
    // Wait for Go Message
...
{
    union CCMessage l_mhHello;
    if ((l_mhHello.mhHello = MessageHello_Initialize (
                             p_pContext->uuidNuggetId,
                             p_pContext->uuidNuggetType,
    ...
    if (!CommandQueue_Put (sg_writeQueue, &l_mhHello))
    {
        rzb_log (LOG_ERR, "%s: C&C Hello Timer: Failed to send Hello Message", __func__);
        MessageHello_Destroy(l_mhHello.mhHello);
        return false;
    }
    MessageHello_Destroy(l_mhHello.mhHello);
    return true;
                                  ...
                                  *mcuConfigUpdate)
{
...
#endif //CNC_DEBUG
    struct RazorbackContext *l_pContext;
    union CCMessage l_mcaConfigAck;
    ...
    pthread_mutex_lock(&list->mutex);
    UUID_Clear_List(list);
    curStr = mcuConfigUpdate->dataTypesNames;
    curUuid = mcuConfigUpdate->dataTypesUuids;
    for (pos =0; pos < mcuConfigUpdate->dataTypesCount; pos++)
    {
        UUID_Add_List_Entry(list, *curUuid, curStr, "");
        curStr = curStr + strlen(curStr) +1;
        curUuid+=1;
    ...
    if ((l_mcaConfigAck.mcaConfigAck = MessageConfigurationAck_Initialize(
    ...
    if (!CommandQueue_Put (sg_writeQueue, &l_mcaConfigAck))
    {
        rzb_log (LOG_ERR, "%s: C&C Config Update: Failed to send Configuration Ack Message", __func__);
        MessageConfigurationAck_Destroy(l_mcaConfigAck.mcaConfigAck);
        return false;
    }
...
Default_processGoMessage (struct MessageGo *mgGo)
{
    ...
    if ((l_mrRunning.mrRunning = MessageRunning_Initialize(
            mgGo->ccmhMessageHeader.uuidDestNugget,
            mgGo->ccmhMessageHeader.uuidSourceNugget)) == NULL)
    {
        rzb_log(LOG_ERR, "%s: Failed to create running message", __func__);
        return false;
    }
    if (!CommandQueue_Put (sg_writeQueue, &l_mrRunning))
    {
        rzb_log (LOG_ERR, "%s: Go Hook: Failed to send running message", __func__);
        MessageRunning_Destroy(l_mrRunning.mrRunning);
        return false;
    }
    MessageRunning_Destroy(l_mrRunning.mrRunning);
    return true;
...
Default_processTermMessage (struct MessageTerminate *mtTerminate)
{
    ...
    union CCMessage l_mbBye;
    if ((l_mbBye.mbBye = MessageBye_Initialize(
            mtTerminate->ccmhMessageHeader.uuidDestNugget,
            mtTerminate->ccmhMessageHeader.uuidSourceNugget)) == NULL)
    {
        rzb_log(LOG_ERR, "%s: Terminate Hook: Failed to alloc bye message", __func__);
    }
    else
    {
        if (!CommandQueue_Put (sg_writeQueue, &l_mbBye))
        {
            rzb_log (LOG_ERR, "%s: Terminate Hook: Failed to send bye message", __func__);
        }
        MessageBye_Destroy(l_mbBye.mbBye);
    }
