...
ConnectedEntityList_Start (void)
{
    ...
    if (timer_create (CLOCK_REALTIME, l_pProps, &tTimer) == -1)
    {
        rzb_perror
    ...
    if (timer_settime (tTimer, 0, &l_itsTimerSpec, NULL) == -1)
    {
...
ConnectedEntityList_GetDispatcher(void)
{
    ...
    for (conf_int_t i = 0; i < localityCount; i++)
    {
        for (node = dispatchers->head; node != NULL; node = node->next)
        {
            entity = node->item;
...
ConnectedEntityList_GetDispatcher_HighestPriority()
{
    ...
    List_Lock(sg_pEntityList);
    List_ForEach(sg_pEntityList, ConnectedEntityList_CollectHighDispatcher, &entity);
    if (ret != NULL)
