...
CommandQueue_Initialize (int flags)
{
    // the name
    ...
    return Queue_Create (l_sQueueName, flags);
}

SO_PUBLIC void
CommandQueue_Terminate (struct Queue *queue)
{
...
static struct CcMessageHeader *
CommandQueue_GrowMessage(struct CcMessageHeader * orig)
{
    struct CcMessageHeader *ret = NULL;
    size_t newSize =0;
    switch (orig->mhHeader.iType)
    {
    case MESSAGE_TYPE_HELLO:
        newSize = sizeof(struct MessageHello);
        break;
    case MESSAGE_TYPE_REG_REQ:
        newSize = sizeof(struct MessageRegistrationRequest);
        break;
    case MESSAGE_TYPE_REG_RESP:
        newSize = sizeof(struct MessageRegistrationResponse);
        break;
    case MESSAGE_TYPE_REG_ERR:
        newSize = sizeof(struct MessageError);
        break;
    case MESSAGE_TYPE_CONFIG_UPDATE:
        newSize = sizeof(struct MessageConfigurationUpdate);
        break;
    case MESSAGE_TYPE_CONFIG_ACK:
        newSize = sizeof(struct MessageConfigurationAck);
        break;
    case MESSAGE_TYPE_CONFIG_ERR:
        newSize = sizeof(struct MessageError);
        break;
    case MESSAGE_TYPE_STATS_REQ:
        newSize = sizeof(struct MessageStatsRequest);
        break;
    case MESSAGE_TYPE_STATS_RESP:
        newSize = sizeof(struct MessageStatsResponse);
        break;
    case MESSAGE_TYPE_STATS_ERR:
        newSize = sizeof(struct MessageError);
        break;
    case MESSAGE_TYPE_PAUSE:
        newSize = sizeof(struct MessagePause);
        break;
    case MESSAGE_TYPE_PAUSED:
        newSize = sizeof(struct MessagePaused);
        break;
    case MESSAGE_TYPE_GO:
        newSize = sizeof(struct MessageGo);
        break;
    case MESSAGE_TYPE_RUNNING:
        newSize = sizeof(struct MessageRunning);
        break;
    case MESSAGE_TYPE_TERM:
        newSize = sizeof(struct MessageTerminate);
        break;
    case MESSAGE_TYPE_BYE:
        newSize = sizeof(struct MessageBye);
        break;
    case MESSAGE_TYPE_CLEAR:
        newSize = sizeof(struct MessageCacheClear);
        break;
    default:
        newSize=0;
    }
    if (newSize == 0)
    {
        rzb_log(LOG_ERR, "%s: Failed to grow message, invalid type", __func__);
        free(orig);
        return NULL;
    }
    if ((ret = realloc(orig, newSize)) == NULL)
    {
        rzb_log(LOG_ERR, "%s: Failed to grow message, malotile disfunction", __func__);
        free(orig);
        return NULL;
    }
    return ret;
...
SO_PUBLIC bool
CommandQueue_Get (struct Queue *queue, union CCMessage *p_pMessage)
{
    ...
    if ((p_pMessage->mchHeader = calloc(1,sizeof(struct CcMessageHeader))) == NULL)
    {
        BinaryBuffer_Destroy (l_pBuffer);
        rzb_log (LOG_ERR,
                 "%s: failed to allocate header structure", __func__);
        return false;
    }
    ...
    if ((p_pMessage->mchHeader = CommandQueue_GrowMessage(p_pMessage->mchHeader)) == NULL)
    {
        BinaryBuffer_Destroy (l_pBuffer);
        rzb_log (LOG_ERR,
                 "%s: failed due to failure of CommandQueue_GrowMessage", __func__);
        return false;
    }
    ...
    case MESSAGE_TYPE_CONFIG_UPDATE:
        ...
        if (!BinaryBuffer_Get_uint32_t(l_pBuffer, &p_pMessage->mcuConfigUpdate->dataTypesCount))
        {
            rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name Count", __func__);
            return false;
        }
        if (!BinaryBuffer_Get_uint32_t(l_pBuffer, &p_pMessage->mcuConfigUpdate->dataTypesNamesSize))
        {
            rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name Names size", __func__);
            return false;
        }
        if (p_pMessage->mcuConfigUpdate->dataTypesCount > 0)
        {
            p_pMessage->mcuConfigUpdate->dataTypesUuids = calloc(p_pMessage->mcuConfigUpdate->dataTypesCount, sizeof(uuid_t));
            p_pMessage->mcuConfigUpdate->dataTypesNames = calloc(p_pMessage->mcuConfigUpdate->dataTypesNamesSize, sizeof(char));
            if ((p_pMessage->mcuConfigUpdate->dataTypesUuids == NULL ) ||
                    (p_pMessage->mcuConfigUpdate->dataTypesNames == NULL ) )
            {
                rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) failed to allocate income message structures", __func__);
                return false;
            }
            if (!BinaryBuffer_Get_ByteArray(l_pBuffer, p_pMessage->mcuConfigUpdate->dataTypesCount * sizeof(uuid_t), (uint8_t*)p_pMessage->mcuConfigUpdate->dataTypesUuids))
            {
                rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) failed to read Name uuids", __func__);
                return false;
            }

            if (!BinaryBuffer_Get_ByteArray(l_pBuffer, p_pMessage->mcuConfigUpdate->dataTypesNamesSize * sizeof(char), (uint8_t*)p_pMessage->mcuConfigUpdate->dataTypesNames))
            {
                rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) failed to read Name names", __func__);
                return false;
            }
...
SO_PUBLIC bool
CommandQueue_Put (struct Queue *queue, union CCMessage *p_pMessage)
{
    ASSERT (p_pMessage != NULL);
    ...
    case MESSAGE_TYPE_CONFIG_UPDATE:
        ...
        if (!BinaryBuffer_Put_uint32_t(l_pBuffer, p_pMessage->mcuConfigUpdate->dataTypesCount))
        {
            rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name Count", __func__);
            return false;
        }
        if (!BinaryBuffer_Put_uint32_t(l_pBuffer, p_pMessage->mcuConfigUpdate->dataTypesNamesSize))
        {
            rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name name size", __func__);
            return false;
        }
        if (p_pMessage->mcuConfigUpdate->dataTypesCount > 0)
        {
            if (!BinaryBuffer_Put_ByteArray(l_pBuffer,p_pMessage->mcuConfigUpdate->dataTypesCount*16,  (uint8_t *)p_pMessage->mcuConfigUpdate->dataTypesUuids))
            {
                rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name uuids", __func__);
                return false;
            }
            if (!BinaryBuffer_Put_ByteArray(l_pBuffer,p_pMessage->mcuConfigUpdate->dataTypesNamesSize,  (uint8_t *)p_pMessage->mcuConfigUpdate->dataTypesNames))
            {
                rzb_log(LOG_ERR, "%s: (CONFIG_UPDATE) Failed to put NTLV Name names", __func__);
                return false;
            }
