...
InspectorQueue_Get (struct Queue *p_pQueue)
{
    ...
    if (!BinaryBuffer_Get_EventId (l_pBuffer, &message->eventId))
    {
        BinaryBuffer_Destroy (l_pBuffer);
        MessageInspectionSubmission_Destroy(message);
        return NULL;
    }
    if (!BinaryBuffer_Get_NTLVList (l_pBuffer, &message->pEventMetadata))
    {
        BinaryBuffer_Destroy (l_pBuffer);
        MessageInspectionSubmission_Destroy(message);
        return NULL;
