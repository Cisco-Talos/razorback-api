...
Queue_Read_Message(struct Socket *socket) 
{
    ...
    if ((l_sBuffer = (char *)calloc(MBUF_SIZE, sizeof(char))) == NULL )
    {
...
readverb:
    ...
    if ((message->sVerb = (char *)calloc(l_iMessageLen, sizeof(char))) == NULL)
    {
    ...
    if (message->bodyLength != 0) // Read the message body
    {
		...
		else if ((message->pBody = (uint8_t *)malloc(message->bodyLength+1)) == NULL) 
        {
			//Need to store it
            rzb_log (LOG_ERR,
