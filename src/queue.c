...
readverb:
    ...
    if (message->bodyLength != 0) // Read the message body
    {
		...
		else if ((message->pBody = malloc(message->bodyLength)) == NULL) 
        {
			//Need to store it
            rzb_log (LOG_ERR,
...
Queue_Create (const char * p_sQueueName, int p_iFlags)
{
    ...
    l_pQueue->mode = MESSAGE_MODE_BIN;
    return l_pQueue;
}
...
Queue_Put (struct Queue * queue,  struct Message * message)
{
    ...
    if (!Message_Add_Header(stompMessage->headers, "receipt", messageId) ||
            !Message_Add_Header(stompMessage->headers, "destination", queue->sName) ||
    ...
    if (!Queue_Send_Message(queue->pWriteSocket, stompMessage))
    {
