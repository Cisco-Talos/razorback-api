 ...
 */
extern bool Hash_IsEqual (const struct Hash *p_pHashA,
 ...
 */
extern char * Hash_ToText (const struct Hash *p_pHash);

/** Create a hash
 * @return a new hash on success NULL on error.
 */
extern struct Hash * Hash_Create (void);
extern struct Hash * Hash_Create_Type (uint32_t p_iType);

/** Return the size of the digest in bytes.
 * @param p_pHash the hash.
 * @return the length in bytes.
 */
extern uint32_t Hash_DigestLength (struct Hash *p_pHash);
extern uint32_t Hash_BinaryLength (struct Hash *p_pHash);

/** Return the size of the string required to hold the hex encoded digets.
 * @param p_pHash the hash.
 * @return the length of the array including null
 */
extern uint32_t Hash_StringLength (struct Hash *p_pHash);
 ...
 */
extern bool Hash_Update (struct Hash *p_pHash, uint8_t * p_pData,
                         uint32_t p_iLength);
 ...
 */
extern bool Hash_Finalize (struct Hash *p_pHash);

/** destroys a hash
 * @param p_pHash the hash
 */
extern void Hash_Destroy (struct Hash *p_pHash);

/** copies a hash
 * @param p_pDestination the destination
 * @param p_pSource the source
 */
extern bool Hash_Copy (struct Hash *p_pDestination,
                       const struct Hash *p_pSource);
