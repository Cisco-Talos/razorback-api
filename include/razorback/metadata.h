 ...
 * @return True on success, false on error.
 * @note This will return the first item in the list with that name.  If there is more than with the same name type tuple then the first one will be returned.
 */
SO_PUBLIC extern bool Metadata_Get_String(
    List_t *list,
    uuid_t name,
    uint32_t *len,
    const char **string
