...
extern bool Message_Register_Handler(struct MessageHandler *);

extern bool Message_Get_Nuggets(struct Message *message, uuid_t source, uuid_t dest);

extern bool Message_Deserialize_Empty(struct Message *message, int mode);
extern bool Message_Serialize_Empty(struct Message *message, int mode);

extern struct Message * Message_Create_Directed(uint32_t type, uint32_t version, size_t msgSize, const uuid_t source, const uuid_t dest);
extern struct Message * Message_Create_Broadcast(uint32_t type, uint32_t version, size_t msgSize, const uuid_t source);
extern void Message_Destroy(struct Message *message);
 ...
 */
extern struct Message* MessageHello_Initialize (struct RazorbackContext *context);
