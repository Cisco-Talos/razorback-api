...
typedef struct
{
    ...
    RZBConfCallBack *callback;  ///< Callback structure.
} RZBConfKey_t;
 ...
 */
extern bool readMyConfig (const char *configDir, const char *configFile,
                          RZBConfKey_t * config);

/** Clean the memory allocated by ::readApiConfig and ::readMyConfig
 *
 */
extern void rzbConfCleanUp (void);

extern config_setting_t *getConfigArray (config_t *config, const char *configFile, const char *name);
extern config_setting_t *getNextConfigElem (config_t *config, config_setting_t *list);
