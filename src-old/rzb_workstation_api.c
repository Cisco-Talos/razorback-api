...
HRESULT getRouteTable(DATA_ENTRY_BLOCK **route_table)
{
    ...
    *route_table = NULL;
    ...
    rzb_log(LOG_ERR, "Getting route table for %s:%s", rzbconfig.dsrvaddr, rzbconfig.dsrvport);
    net = ConnectMe(rzbconfig.dsrvaddr, rzbconfig.dsrvport);
    if (!net)
    {
        rzb_log(LOG_ERR, "connection to %s:%s failed", rzbconfig.dsrvaddr, rzbconfig.dsrvport);
    ...
    if (sendWrap(net, &scpacket, sizeof(scpacket)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed snedwrap of sc");
        NetworkConnectionDestroy(net);
        return R_FAIL;
    }
    ...
    if (sendWrap(net, &req, sizeof(req)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed sendWrap of req");
        NetworkConnectionDestroy(net);
        return R_FAIL;
    }
    ...
    if (recvWrap(net, &resp, sizeof(resp)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed recvwrap of resp");
        NetworkConnectionDestroy(net);
        return R_FAIL;
    }
    ...
    if (resp.resp_type != ROUTE_TABLE_START)
    {
        ...
        {
            rzb_log(LOG_ERR, "Response type not route table start");
            NetworkConnectionDestroy(net);
            return R_FAIL;
        }
    }

    rzb_log(LOG_INFO, "Receiving route table");
    if (recvWrap(net, &resp, sizeof(resp)) != R_SUCCESS)
    {
        NetworkConnectionDestroy(net);
        return R_FAIL;
    ...
    *route_table = de;
    ...
    {
        rzb_log(LOG_INFO, "Route data entry...");
...
static HRESULT getDataEntry(RZBNetworkConnection *net, WKST_DATA_RESPONSE *resp, DATA_ENTRY_BLOCK *de)
{
    ...
    {
        rzb_log(LOG_ERR, "Failed to receive data entry");
        return R_FAIL;
    }
    ...
    if (recvWrap(net, resp, sizeof(*resp)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed to receive next data response after data entry");
        return R_FAIL;
    }
    ...
    while (resp->resp_type == ROUTE_APP_ENTRY)
    {
        ...
        {
            rzb_log(LOG_ERR, "Failed in getAppEntry");
            while ((ae = de->app_list))
            {
                de->app_list = ae->next;
                freeAppEntryBlock(ae);
...
static HRESULT getAppEntry(RZBNetworkConnection *net, WKST_DATA_RESPONSE *resp, APP_ENTRY_BLOCK *ae)
{
    ...
    if (recvWrap(net, &data, sizeof(data)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "failed to receive route-header-packet");
        return R_FAIL;
    }
    ...
    if (recvWrap(net, resp, sizeof(*resp)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed to get wkst_data_response in getAppEntry");
        return R_FAIL;
    }
...
static HRESULT getNuggetEntry(RZBNetworkConnection *net, WKST_DATA_RESPONSE *resp, ROUTE_ENTRY_BLOCK *re)
{
    ...
    if (recvWrap(net, &data, sizeof(data)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed to get route nugget packet");
        return R_FAIL;
    }

    if (recvWrap(net, resp, sizeof(*resp)) != R_SUCCESS)
    {
        rzb_log(LOG_ERR, "Failed to get response packet in getNuggetEntry");
        return R_FAIL;
    }
